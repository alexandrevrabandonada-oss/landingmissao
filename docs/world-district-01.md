...
Na décima terceira evolução, a extensão jogável passou de 11,7 para 31,75 unidades. O Memorial foi deslocado para `z = -5,6`, o Comum para `z = -16,4` e a Central para `z = -27,2`; a velocidade aumentou moderadamente para 3,15 unidades por segundo, alongando a descoberta sem transformar a navegação assistida em espera. Câmera, atmosfera, fog, iluminação, mapa, limites e detecção de zonas agora compartilham a mesma função de progresso territorial.

A composição espacial foi dividida em três experiências que não cabem mais no mesmo enquadramento. O Núcleo Brutalista usa volumes altos, assimetria, sombras e ferrugem para comprimir o começo. O Corredor do Mutirão repete pórticos, bases habitáveis, bandeiras, iluminação e vegetação crescente como uma arquitetura em construção. O Jardim Solar abre a escala com coberturas fotovoltaicas, captação de água, árvores, rotores e uma linha verde que conduz à Central de Missões. Toda a expansão estrutural nova usa apenas três `InstancedMesh`: caixas arquitetônicas, colunas cilíndricas e copas/símbolos icosaédricos. Totens, postes, árvores, bandeiras, canopies e sombras existentes receberam novas instâncias sem criar materiais ou lotes adicionais.

O HUD passou a nomear o trecho atual — Núcleo Brutalista, Corredor do Mutirão ou Jardim Solar — e mostrar a porcentagem física da travessia sem substituir o progresso semântico dos três marcos. O mapa usa a nova escala longitudinal, mantém os três destinos distribuídos e reposiciona o rótulo do Comum para não cortar em 320 px. Pórticos sucessivos, linha cromática do piso e luzes laterais fornecem orientação no próprio mundo; bússola, distância e rota assistida permanecem como confirmação acessível.

O playtest percorreu os três marcos e suas transições em 1366 × 768 e 390 × 844, além de validar o mapa em 320 × 568. Não houve overflow nem alvo interativo menor que 44 px. No boot do Núcleo Brutalista, a cena registrou 102 draw calls e 10.055 triângulos; o aumento inclui os três novos lotes e batches animados cujo bounding volume agora atravessa todo o distrito. O orçamento continua abaixo da composição pré-instanciamento e sem física, sombras dinâmicas, texturas ou pós-processamento. O único aviso de console permanece a depreciação conhecida de `THREE.Clock`.

Na décima quarta evolução, atmosfera e câmera passaram a marcar três atos distintos. O começo mantém fog curto, céu frio e luz industrial; o mutirão ganha âmbar, maior alcance e uma abertura temporária de câmera; o jardim conclui a transição com fog distante, luz quente e campo visual mais amplo. O cálculo continua imperativo no controlador existente, sem adicionar callbacks, luzes ou pós-processamento.

Os batches estruturais da v13 receberam rasgos de luz nos monólitos, mesas e bancos comunitários, canais d'água, pedras de passagem e detalhes solares sem criar draw calls. O playtest revelou que peças do jardim antigo ficavam entre a câmera e o Comum depois do alongamento da rota; canopies, bandeiras e o antigo gateway foram retirados desse corredor ou movidos para trás do marco. O pórtico principal foi elevado para preservar a linha de visão do avatar e do beacon.

As entradas da Zona 02 e Zona 03 agora apresentam uma instrução transitória curta, vinculada à linguagem do cenário, sem adicionar HUD permanente. Em 1366 × 768 e 390 × 844, chegada, interação e percurso permaneceram legíveis e sem overflow ou alvos menores que 44 px. A sessão com o GLB carregado no Jardim Solar registrou 105 draw calls e 9.981 triângulos; a diferença de estado e frustum impede comparação direta com o boot, enquanto o custo estrutural desta evolução permanece igual ao da v13.

Na décima quinta evolução, o avatar passou a usar 80% da escala visual anterior. Posição de simulação, velocidade, colisão, raio de interação e composição de chegada não foram alterados; a mudança amplia a relação aparente entre personagem, arquitetura e horizonte sem comprometer controles ou identidade visual.

Os três marcos passaram a declarar uma utilidade cívica concreta. O Memorial funciona como arquivo público, o Comum como assembleia e a Central de Missões como central de ação. Cada função tem três possibilidades explícitas, disponíveis no diário de campo, no mapa narrativo e no modo leve. No 3D, três estações laterais com suporte, módulos cromáticos e sinais facetados materializam essas funções sem ocupar o eixo caminhável. Todas as peças reutilizam os batches de caixas e icosaedros da v13, sem novos materiais, assets, callbacks ou draw calls.

O playtest validou a escala inicial, a rota assistida, a chegada e o diário do Memorial, além do mapa e do modo leve. No Chrome com viewport móvel solicitado em 390 × 844, a área útil medida foi 355 × 768, sem overflow horizontal e sem botão visível menor que 44 px; mapa e diário ocuparam a largura disponível, mantiveram fechamento acessível e rolagem interna. O único aviso de console continuou sendo a depreciação conhecida de `THREE.Clock`.

Na décima sexta evolução, a jornada deixou de terminar na borda do mundo 3D. Um Caderno de Jornada global combina, somente no navegador, os marcos visitados no distrito e as etapas já percorridas no portal. A superfície permanece invisível para uma visita nova e surge apenas depois de uma escolha ou descoberta significativa, preservando a hierarquia editorial da abertura.

O caderno apresenta sete sinais — três marcos territoriais e quatro etapas de participação — e recomenda uma continuidade coerente com o estado atual. Memória conduz às pautas; Comum e Central orientam à escolha de missão; uma missão escolhida retoma seu canal real de ação. Atalhos secundários mantêm Explorar, Pautas e Participar disponíveis sem transformar o painel em um novo menu principal.

Para proteger as experiências de tela cheia, o caderno não é montado visualmente em `/explorar`, `/jogo` ou `/apoio`. A sincronização entre abas usa o evento nativo de armazenamento; atualizações na mesma aba usam eventos locais explícitos. Reset, seleção de missão, visita de marco e mudança de rota atualizam a recomendação sem conta, localização, nome ou dado pessoal. O painel cabe em `100svh`, respeita safe areas, oferece alvos de pelo menos 44 px, fecha com Escape e remove animação quando o sistema solicita movimento reduzido.

Na décima sétima evolução, a Central de Missões deixou de selecionar apenas a participação pelo celular. O marco final agora apresenta os quatro caminhos reais do portal — celular, rua, contribuição e compartilhamento — com estado selecionado, próximo passo e CTA correspondente. A escolha é persistida pelo mesmo armazenamento da landing e atualiza imediatamente o Caderno de Jornada, permitindo sair do mundo, retornar ao portal e continuar com a mesma missão marcada.

Landing e mundo 3D passaram a construir essas opções a partir de uma única fonte de conteúdo e URLs rastreáveis. Isso evita divergência de texto, destino ou comportamento externo entre as duas superfícies. O painel genérico de utilidade foi removido apenas do marco final por repetir a função da central interativa; Memorial e Comum mantêm seus blocos informativos.

No playtest em modo leve, a seleção feita no distrito reapareceu corretamente na landing e o caderno recomendou o mesmo canal de ação. Em 390 × 844, as quatro escolhas, o resumo e o CTA couberam no painel sem rolagem interna, com alvos mínimos de 48 px. A largura mobile passou a usar limites `left/right` da área útil em vez de `100vw`, eliminando o deslocamento causado por navegadores que incluem a barra de rolagem no cálculo de viewport.

Na décima oitava evolução, a conclusão deixou de devolver o jogador a um objetivo genérico. Depois dos três marcos e de uma escolha na Central, o HUD se transforma em Plano de Ação persistente: nomeia a missão, resume o próximo passo e oferece o mesmo destino real da landing. A seleção também dispara uma confirmação transitória curta, enquanto o plano permanece disponível de forma recolhível quando o jogador retoma o controle.

Essa continuidade não adiciona um novo modal nem ocupa o centro do playfield. O mesmo cartão de objetivo muda de função, mantém os três marcos registrados e expõe um CTA de 46 px apenas na área expandida. O playtest em 390 × 844 confirmou largura útil de 310 px, ausência de overflow horizontal, retorno ao estado recolhido e link interno correto para compartilhamento. Missões externas preservam nova aba, parâmetros de origem e rastreamento específico do HUD.

Na décima nona evolução, a jornada passou a reconhecer a escala de uma candidatura a deputado estadual. Seis pautas cobrem saúde, mobilidade, trabalho, educação e cultura, moradia e clima, direitos e democracia em diferentes territórios do Rio de Janeiro. A seleção fica no navegador, atualiza o Caderno de Jornada e orienta a escolha de uma missão sem transformar a página em enquete ou promessa pronta.

O cartão compartilhável agora combina forma de participação e pauta estadual, usa a mensagem “Minha missão pelo Rio de Janeiro” e preserva Volta Redonda como origem política. O marco do Comum também passou a ligar sua memória local às políticas estaduais e à circulação entre territórios.

### v20 — pauta estadual transforma o distrito

A escolha feita em `/pautas` agora acompanha a pessoa até o distrito. A cor da pauta reaproveita o arco, a linha de rota, os símbolos e o farol da Assembleia do Comum; nenhum objeto, material ou draw call foi adicionado. O objetivo contextual, o mapa narrativo, o diário de campo e o modo leve passam a apresentar a pauta selecionada como assembleia estadual e a Central de Missões como caminho de ação. Assim, o cenário deixa de ser apenas ambientação e passa a responder à intenção política da pessoa com equivalência entre 3D e modo acessível.
